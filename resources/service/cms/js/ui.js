...
$(window).load(function(){
    ...
    $('.function .favorites > a').on('click', function(){
        if($(this).hasClass('on')){
            $(this).removeClass('on');
            $(this).next().hide();
        } else{
	...
	//table td 안에 input, select 들어가는 경우 padding값 조절
	var inputCell = $('table tbody td').find('input').length;
	if(inputCell){		
		//console.log('y');
		// $('input[type="text"]').parents('table tbody td').addClass('pd');
		// $('input[type="password"]').parents('table tbody td').addClass('pd');
		// $('input[type="tel"]').parents('table tbody td').addClass('pd');
		// $('input[type="number"]').parents('table tbody td').addClass('pd');
		// $('input[type="email"]').parents('table tbody td').addClass('pd');
        // $('input[type="file"]').parents('table tbody td').addClass('pd');
		$('input').parents('table tbody td').addClass('pd');
		$('select').parents('table tbody td').addClass('pd');
		$('textarea').parents('table tbody td').addClass('pd');
        $('.btn').parents('table.row tbody td').addClass('pd');        
	} else{
		//console.log('n');
		return false;
	}	
    
    //checkbox, radio 여백 없애기
    $('.chkWrap').parent().addClass('ft0');
    $('.radioWrap').parent().addClass('ft0');
});
...
function snbFn(){
    $('.snb .depth1').on('click', function(e){
		if($(this).hasClass('active')){
			$(this).removeClass('active');
			$(this).next('.depth2').stop().slideUp({duration:400, easing:"easeOutBack" });
            $(this).children('.gubun').text('+');
		} else{
			if($(this).hasClass('over')){
				...
				if($(this).next('.depth2').length){
                    ...
                    $(this).addClass('active');
                    $('.snb .depth2').hide();
                    $(this).next('.depth2').stop().slideDown({duration:400, easing:"easeOutBack"});                    
                    $('.snb .gubun').text('+');
                    $(this).children('.gubun').text('-');                    
	...
	$('.snb .depth3 > ul > li > a').on('click', function(){
		if($(this).hasClass('active')){
			...
			$(this).addClass('active');
			$('.snb .depth4').hide();
			if($(this).next('.depth4').length){
				$(this).next().stop().slideDown({duration:400, easing:"easeOutBack"})
                $('.snb .depth3 .gubun').text('+');
                $(this).children('.gubun').text('-');
			}
		}
	});
