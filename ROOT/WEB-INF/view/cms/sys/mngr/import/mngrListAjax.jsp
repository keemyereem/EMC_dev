...
<c:if test="${not empty webResult.listPage.listMap }" >
    <c:forEach var="item" items="${webResult.listPage.listMap }" varStatus="i" >
	   <tr>
           ...
           <td ><a href="javascript:onModifyForm('${item.mngrId}')"><c:out value="${item.mngrId}" /></a></td>
           <td ><c:out value="${item.mngrExpsrNm}" /></td>
           <td ><c:out value="${item.deptNm}" /></td>           
           <td ><c:out value="${item.clsfNm}" /></td>
           <td ><c:out value="${item.useAt}" /></td>
           <td ><c:out value="${item.mngrAcntSttusNm}" /></td>
           <td><fmt:formatDate type="date" pattern="yyyy-MM-dd HH:mm:ss" value="${item.lastLoginDt}" /></td>           
       </tr>
    </c:forEach>
