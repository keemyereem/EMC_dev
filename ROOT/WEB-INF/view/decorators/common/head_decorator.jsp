...
<script>
var memberCountConTxt1_prev= '<c:out value="${retMap.prevCarbonPercentage}" />';    // 이전값
var memberCountConTxt1= '<c:out value="${retMap.carbonPercentage}" />';    // 갱신된 변수값 저장
var memberCountConTxt2= '<c:out value="${retMap.totalPine }" />';    // 갱신된 변수값 저장
var memberCountConTxt2_prev= '<c:out value="${retMap.prevTotalPine}" />';    // 이전값
</script>
...
<script type="text/javascript">
  let Globals_MainPage="<c:out value="${Globals.MAIN_PAGE}" />" ;
  let Globals_Login_Url="<c:out value="${Globals.LOGIN_URL}" />" ;
  // js파일에서 사욯할 문자
  ...
  const _contextPath = "<c:out value="${pageContext.request.contextPath }" />" ;
