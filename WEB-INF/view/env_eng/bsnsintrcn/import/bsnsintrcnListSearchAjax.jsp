...
<c:forEach var="item" items="${bsnsintrcnList.listPage.listMap}" varStatus="i" >
	...
	<c:if test="${item.langCode eq 'C001002'}"><c:set var="ctgryNm" value="${item.ctgryCodeEngNm}" /></c:if>
...
$(function () {
    ...
    $("#valTotalRecordCount").val('<c:out value="${bsnsintrcnList.listPage.paging.totalRecordCount}" />');
});
</script>
