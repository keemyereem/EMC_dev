            ...
            <div class="contents cont_business  cont_business_en">
                <div class="contents_inner onm_inner">
                    ...
                    <h3 class="title" data-aos="fade-up" data-aos-offset="200">
                    	<c:if test="${webResult.params.searchBsnsintrcnCode eq 'A012001'}">Environmental infrastructure O&M</c:if>
                        <c:if test="${webResult.params.searchBsnsintrcnCode eq 'A012002'}">Construction</c:if>
                    </h3>
                    <div class="tit_line"></div>
                    <div class="business_contents business_contents_en sub_contents">
                        <div class="inner">
                            <section class="section1">
                                <p class="sub_con">
                                <c:choose>
                                    <c:when test="${webResult.params.searchBsnsintrcnCode eq 'A012001'}">
                                        As a leader in the operation and management of the environmental infrastructure in Korea, We are engaged in various projects to help people lead happy lives in a fresh and cleaner environment.
                                    </c:when>
                                    <c:otherwise>
                                        With over 20 years of experience in design, construction and operations since the company’s establishment in 1997, we have been devoted to investing in more efficient and economic treatment technologies. 
                                        We preserve the value of public assets by actively participating in projects for the construction of new facilities and the rehabilitation and maintenance of aged facilities.
                                    ...
                                    <li class="<c:if test="${i.count eq 1 }">on</c:if>">
                                        <a href="javascript:fnCategoryList('<c:out value="${item.code}" />','<c:out value="${i.count}" />');">
                                            ...
                                            <div class="icon">
                                                <c:choose>
                                    			<c:when test="${webResult.params.searchBsnsintrcnCode eq 'A012001'}">
                                                <img src="/resources/service/env/images/business/onm_icon0<c:out value="${i.count}" />.png" alt="onm_icon0<c:out value="${i.count}" />" class="onm_icon">
                                                <img src="/resources/service/env/images/business/onm_icon0<c:out value="${i.count}" />_on.png" alt="onm_icon0<c:out value="${i.count}" />_on" class="onm_icon">
                                                </c:when>
			                                    <c:otherwise>
			                                    <img src="/resources/service/env/images/business/envir_icon0<c:out value="${i.count}" />.png" alt="envir_icon0<c:out value="${i.count}" />" class="envir_icon">
                                                <img src="/resources/service/env/images/business/envir_icon0<c:out value="${i.count}" />_on.png" alt="envir_icon0<c:out value="${i.count}" />_on" class="envir_icon">
			                                    </c:otherwise>
			                                 </c:choose>
                                            </div>
                                            <p class="nav_tit">
                            ...
                            <section class="section2">
                                ...
                                <c:choose>
                             	<c:when test="${webResult.params.searchBsnsintrcnCode eq 'A012002'}">
                             		<%@ include file="/WEB-INF/view/env_eng/bsnsintrcn/import/section2_2.jsp" %>
                             	</c:when>
                             	<c:otherwise>
                             		<%@ include file="/WEB-INF/view/env_eng/bsnsintrcn/import/section2_1.jsp" %>
                             	</c:otherwise>
                             	</c:choose>
                ...
                <section class="bus_section3">
                    <div class="search">
                        <div class="input_row input_search">
                            <input type="text" id="searchText" name="searchText" value="<c:out value="${webResult.params.searchText}" />" placeholder="Search" />
...
<script>
    ...
    function fn_viewMoreResult() {
        ...
        //if (parseInt($('#currentPageNo').val()) >= parseInt(totalCnt) ) {
    ...
    function fnCategoryList(searchClCode, no){
    	...
    	fnSubCategoryList(searchClCode,'1');
