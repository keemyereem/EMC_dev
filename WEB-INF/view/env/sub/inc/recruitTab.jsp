...
<script>
    ...
    <c:set var="tabIndex"><c:out value="${empty webResult.params.tabIndex ? '1' : webResult.params.tabIndex}"/></c:set>

                        <div class="tab_list">
                            <ul class="nav_btn" data-aos="fade-up" data-aos-duration="600" data-aos-offset="200">
                                <li class="${tabIndex eq '1' ? 'on focus' : ''}"><a href="javascript:fn_moveTab(1, '', '5', 'BM0000000004');">채용공고</a></li>
                                <li class="${tabIndex eq '2' ? 'on focus' : ''}"><a href="javascript:fn_moveTab(2, 'recruit_process', '5', '');">채용 프로세스</a></li>
                                <li class="${tabIndex eq '3' ? 'on focus' : ''}"><a href="javascript:fn_moveTab(3, 'recruit_intro', '5', '');">직무 소개</a></li>
                                <li class="${tabIndex eq '4' ? 'on focus' : ''}"><a href="javascript:fn_moveTab(4, 'recruit_faq', '5', '');">채용 FAQ</a></li>
                            </ul>
                        </div>
...
// MENUID=ethical_management&MENUNO=3
	function fn_moveTab(tabIndex, menuId, menuNo, bbsNo) {
		...
		let moveUrl = '<c:url value="/sub.do" />';

		if (tabIndex == "1") {
			moveUrl = '<c:url value="/bbs/data/bbsDataList.do" />';
		}
